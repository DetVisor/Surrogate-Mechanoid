...
[HarmonyPatch]
public static class MechFriendly_JobDriver_PlayWalking
{
	private static MethodBase TargetMethod()
	{
		return AccessTools.Method(typeof(JobDriver_PlayWalking), "<Interact>b__12_1");
	}
	...
	private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator il)
	{
		...
		foreach (CodeInstruction t in instructions)
		{
			if (t.Calls(AccessTools.Method(typeof(Pawn_InteractionsTracker), "TryInteractWith")))
			{
				...
				yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(MechFriendly_JobDriver_PlayWalking), "IsSurrogate"));
